...
export default function SignUp() {
  ...
  async function handle() {
    ...
    try {
      ...
      Alert.alert('สมัครสำเร็จ', 'เข้าสู่ระบบเพื่อใช้งาน');
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Text style={{ color: colors.sectionTitle, fontSize: 20, fontWeight: '700', marginBottom: 12 }}>Sign Up</Text>
      <TextInput placeholder='Name' value={name} onChangeText={setName} style={[styles.input, { color: colors.text }]} placeholderTextColor='#999' />
      ...
      <TouchableOpacity style={[styles.btn, { backgroundColor: colors.accent }]} onPress={handle} disabled={loading}>
        <Text style={{ color: colors.skillText, fontWeight: '700' }}>{loading ? 'กำลัง...' : 'Sign Up'}</Text>
      </TouchableOpacity>
    </View>
  );
}
